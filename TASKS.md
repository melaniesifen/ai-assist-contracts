...
- [ ] `ARCH-003`: Add explicit contract version field or negotiation helper where service boundaries need version checks.
- [ ] `ARCH-003`: Add unsupported contract version validation that maps to `UNSUPPORTED_CONTRACT_VERSION`.
- [ ] `ARCH-003`: Add coordinated rollout note requirements for breaking contract changes.
...
- [ ] `AUTH-002`: Define shared typed errors for unauthorized, expired, and malformed product credentials if missing from service needs.
...
- [ ] `EVT-001`: Add HTTP command request/response envelope contracts with request and correlation IDs.
- [ ] `ACTION-002`: Add invalid proposed-action transition validation if lifecycle enforcement remains shared.
...
- [ ] `OPS-003`: Reference metadata-only logging rules for contracts that touch sensitive content.
